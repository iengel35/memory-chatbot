...
ChatLogic::~ChatLogic()
{
    ...
    // chatbot instance automatically deleted once out of scope
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                          ...
                          auto parent_itr = std::find_if(_nodes.begin(), _nodes.end(), [&parentToken](const std::unique_ptr<GraphNode>& node) { return node->GetID() == std::stoi(parentToken->second); });
                          auto child_itr = std::find_if(_nodes.begin(), _nodes.end(), [&childToken](const std::unique_ptr<GraphNode>& node) { return node->GetID() == std::stoi(childToken->second); });
    ...
    std::unique_ptr<ChatBot> local_bot = std::make_unique<ChatBot>("../images/chatbot.png");
    SetChatbotHandle(local_bot.get());
